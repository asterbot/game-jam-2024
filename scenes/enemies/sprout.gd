...
func attack():
	...
	for attack_offset in [-15, 0, 15]:
		var projectile = seed_scene.instantiate() as RigidBody2D
		...
		projectile.position = Vector2(position.x, position.y) + attack_direction*40
		projectile.linear_velocity = attack_direction*(860 + sin(attack_angle - PI)*460)
